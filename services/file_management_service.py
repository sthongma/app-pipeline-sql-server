...
class FileManagementService:
    ...
    def find_folders_with_files(self, root_path: str) -> Dict[str, Dict[str, Any]]:
        """
        ค้นหาโฟลเดอร์ที่มีไฟล์อยู่และจัดกลุ่มตามประเภท (logic type)
        
        Args:
            root_path: โฟลเดอร์ที่ต้องการค้นหา
            
        Returns:
            Dict[str, Dict[str, Any]]: ข้อมูลโฟลเดอร์ที่มีไฟล์ จัดกลุ่มตาม logic type
            {
                'logic_type': {
        ...
        for dirpath, dirnames, filenames in os.walk(root_path):
            if filenames:  # โฟลเดอร์มีไฟล์
                # ตรวจหาประเภทไฟล์จากชื่อไฟล์
    ...
    def _detect_folder_logic_type(self, folder_path: str, filenames: List[str]) -> str:
        """
        ตรวจสอบประเภท logic type ของโฟลเดอร์จากชื่อโฟลเดอร์และชื่อไฟล์
        
        Args:
            folder_path: ที่อยู่โฟลเดอร์
            filenames: รายการชื่อไฟล์ในโฟลเดอร์
            
        Returns:
            str: ประเภท logic type ที่ตรวจพบ
        """
        # ตรวจสอบจากชื่อโฟลเดอร์
        ...
        # ถ้าไม่พบจากไฟล์ ให้ใช้ชื่อโฟลเดอร์
        if folder_name:
    ...
    def move_folders_by_logic_type(self, folder_info: Dict[str, Dict[str, Any]], 
        ...
        """
        ย้ายโฟลเดอร์ที่มีไฟล์โดยจัดกลุ่มตาม logic type เหมือน move_uploaded_files
        
        Args:
            folder_info: ข้อมูลโฟลเดอร์ที่ได้จาก find_folders_with_files
            src_root: โฟลเดอร์ต้นทาง
            dest_root: โฟลเดอร์ปลายทาง
            
        Returns:
            Dict[str, List[Tuple[str, str]]]: ผลลัพธ์การย้ายจัดกลุ่มตาม logic type
        """
        moved_results = {}
        ...
        for logic_type, info in folder_info.items():
            ...
            # สร้างโฟลเดอร์ปลายทางตาม logic type (เหมือน move_uploaded_files)
            ...
            for folder_path in info['folders']:
                try:
                    ...
                    # สร้างชื่อโฟลเดอร์ใหม่พร้อม timestamp หากซ้ำ
    ...
    def archive_old_files(self, source_path: str, archive_path: str, days: int = 30, 
        ...
        """
        ฟังชั่นหลักสำหรับการจัดเก็บไฟล์เก่า (ใช้ logic type เสมอเพื่อความปลอดภัย)
        ...
        try:
            ...
            # ค้นหาและย้ายโฟลเดอร์ที่มีไฟล์ตาม logic type (ใช้เสมอเพื่อป้องกันการลบไฟล์ที่ไม่เกี่ยวข้อง)
    ...
    def analyze_folder_structure(self, root_path: str) -> Dict[str, Any]:
        """
        วิเคราะห์โครงสร้างโฟลเดอร์และแสดงสรุปข้อมูล
        
        Args:
            root_path: โฟลเดอร์ที่ต้องการวิเคราะห์
            
        Returns:
            Dict[str, Any]: รายงานการวิเคราะห์
        """
        analysis = {
            "empty_directories": [],
            "folders_with_files": {},
    ...
    def print_folder_analysis(self, analysis: Dict[str, Any]) -> None:
        """
        แสดงผลการวิเคราะห์โฟลเดอร์ในรูปแบบที่อ่านง่าย
        
        Args:
            analysis: ผลการวิเคราะห์จาก analyze_folder_structure
        """
        if "error" in analysis:
            print(f"❌ เกิดข้อผิดพลาด: {analysis['error']}")
            return
        
        summary = analysis.get("summary", {})
        
        print("=" * 50)
        print("📊 สรุปการวิเคราะห์โครงสร้างโฟลเดอร์")
        print("=" * 50)
        print(f"📁 โฟลเดอร์ว่าง: {summary.get('total_empty_directories', 0)}")
        print(f"📂 โฟลเดอร์ที่มีไฟล์: {summary.get('total_folders_with_files', 0)}")
        print(f"📄 ไฟล์ทั้งหมด: {summary.get('total_files', 0)}")
        print(f"🏷️  ประเภทที่พบ: {', '.join(summary.get('logic_types_found', []))}")
        
        # แสดงรายละเอียดแต่ละประเภท
        folder_info = analysis.get("folders_with_files", {})
        if folder_info:
            print("\n" + "=" * 50)
            print("📋 รายละเอียดตามประเภท")
            print("=" * 50)
            
            for logic_type, info in folder_info.items():
                print(f"\n🏷️  ประเภท: {logic_type}")
                print(f"   📂 โฟลเดอร์: {len(info['folders'])}")
                print(f"   📄 ไฟล์: {info['count']}")
                
                # แสดงโฟลเดอร์แต่ละตัว (จำกัด 3 รายการแรก)
                for i, folder in enumerate(info['folders'][:3]):
                    print(f"   📁 {os.path.basename(folder)}")
                
                if len(info['folders']) > 3:
                    print(f"   ... และอีก {len(info['folders']) - 3} โฟลเดอร์")
